...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(value = RuntimeException.class) // Lỗi runtime exception ở bất cứ nơi nào trong chương
                                                               // trình đều sẽ được bắt ở đây
    ResponseEntity<MessageResponse> handlingRuntimeException(RuntimeException exception) {
        return ResponseEntity.badRequest().body(new MessageResponse(exception.getMessage())); // Loi do du lieu nguoi dung nhap vao se la
                                                                         // badRequest
    }
