...
public class CourtManagerService {
    ...
    public TotalRevenueInformation getTotalRevenueInfo(DashBoardRequest dashBoardRequest){
        ...
        if(!paymentList.isEmpty()){ // xu ly truong hop ca 2 thang khong co doanh thu
            totalAmountCurrentMonth = getTotalAmountPerMonth(dashBoardRequest.getMonth(), dashBoardRequest.getYear(), paymentList);
            totalAmountPreviousMonth = getTotalAmountPerMonth(dashBoardRequest.getPreviousMonth(), dashBoardRequest.getPreviousYear(), paymentList);
    ...
    */
    public int getPercentVariation(int totalAmountCurrentMonth, int totalAmountPreviousMonth){
        double rateOfChange = (totalAmountCurrentMonth-totalAmountPreviousMonth)/(double)totalAmountPreviousMonth * 100;
     ...
     * @param year nam can tinh, ket hop voi thang ra mot don vi thoi gian can tinh tong doanh thu
     * @param paymentList lay ra cac chi tiet hoa don dua tren month va year de tinh tong doanh thu
    ...
    */
    public int getTotalAmountPerMonth(int month, int year, List<QueryTotalRevenueMapper> paymentList){
        ...
        for(QueryTotalRevenueMapper queryTotalRevenueMapper: paymentList){
            if((queryTotalRevenueMapper.getPaymentTime().getMonthValue() == month) && (queryTotalRevenueMapper.getPaymentTime().getYear() == year)){
    ...
    public TotalBookingSlotInformation getTotalBookingSlot(DashBoardRequest dashBoardRequest){
        ...
        if(!bookingTimeList.isEmpty()){
            currentMonthBSQuantity = getBookingSlotQuantity(dashBoardRequest.getMonth(), dashBoardRequest.getYear(), bookingTimeList);
            previousMontBSQuantity = getBookingSlotQuantity(dashBoardRequest.getPreviousMonth(), dashBoardRequest.getPreviousYear(), bookingTimeList);
